/**
 * Interface for skills and languages with ID and name
 */
interface NamedItem {
  id: string;
  name: string;
 ...
 */
function generateConsistentId(text: string): string {
 ...
 */
export const processNamedItems = (items: Array<string | Partial<NamedItem>>): NamedItem[] => {
  ...
  const result = items.map(item => {
    ...
    return null;
  }).filter(Boolean) as NamedItem[]; // Filter out null items
