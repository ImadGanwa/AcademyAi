...
import { Box, Typography, Paper, Tabs, Tab, IconButton, Menu, MenuItem, Chip, Button } from '@mui/material';
...
export const Notifications: React.FC = () => {
  ...
  const handleAction = (notification: Notification) => {
    if (notification.type === 'course' && notification.relatedId) {
      const courseUrl = `/${i18n.language}/courses/${notification.relatedId}`;
  ...
  return (
    <NotificationsContainer>
      ...
      >
        <MenuItem onClick={handleMarkAsRead} sx={{ color: 'primary.main' }}>
          <CheckCircleIcon sx={{ mr: 1 }} /> {t('notifications.markAsRead')}
        </MenuItem>
        <MenuItem onClick={handleDelete} sx={{ color: 'error.main' }}>
          <DeleteIcon sx={{ mr: 1 }} /> {t('notifications.delete')}
        </MenuItem>
      </Menu>
    </NotificationsContainer>
  );
